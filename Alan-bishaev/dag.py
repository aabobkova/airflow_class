...
dag = DAG('kaggle_table_dag', default_args=default_args, schedule_interval='@daily')
...
process_table = PythonOperator(
    ...
    python_callable = func("res.csv"),
    op_args=['input_table.csv', 'output_processed_table.csv'],
    dag=dag
)
...
send_email = EmailOperator(
    ...
    subject='Tabl Processing Results',
    html_content='Attached is the processed table',
    files=['output_precessed_table.csv'],
    dag=dag
)
